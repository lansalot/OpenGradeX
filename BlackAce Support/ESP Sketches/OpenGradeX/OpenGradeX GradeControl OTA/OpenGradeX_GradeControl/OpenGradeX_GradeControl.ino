...
#define DEBUG Serial
#define RTK Serial1
#define BUILTIN_LED D8  // 2 for ESP32 Dev kit, D8 for Xiao ESP32C3
...
uint16_t analogOutput2 = VALVE_NEUTRAL; //send to MCP4725
int cut1 = -1;
...
void loop() {  //Loop triggers every 50 msec (20hz) and sends back offsets Pid ect
  ...
  if (currentTime - lastTime >= LOOP_TIME) // 20 HZ
  {  
    watchdogTimer++;
...
bool SetupGradeControlModule()
{
  ...
  Dac1.setVoltage(VALVE_NEUTRAL, false);
  Dac2.setVoltage(VALVE_NEUTRAL, false); 
...
void SetOutput()
{
  if (isAutoActive && isCutting && isOGXConnected){    
    ...
    if (b_cutDelta < b_deadband){
      analogOutput1 = VALVE_NEUTRAL;
      voltage = ((double)VALVE_NEUTRAL/4096) * 5.0;
      voltage2 =((double)VALVE_NEUTRAL/4096) * 5.0;
    }
    else
    {
...
void SetValveLimits(){
  ...
  switch(gradeConfig.b_valveType) {
    ...
    case CNH:
      retDeadband = VALVE_NEUTRAL - ((gradeConfig.b_retDeadband/200.0)*4096);
      extDeadband = VALVE_NEUTRAL + ((gradeConfig.b_extDeadband/200.0)*4096);    
      retMin = (CNH_MIN * 4096);
      extMax = (CNH_MAX * 4096);
      break;
...
{ 
  ...
  if (packetSize) {       // If we received a package
    ...
    DEBUG.printf("\r\nMessage Received from IP-> %s Port-> %u ", senderIP.toString(), senderPort);
    ...
    switch (header)
    {
      ...
      case SETTINGS_HEADER:
        DEBUG.println("UDP SETTINGS FOUND!");      
      ...
      case SYSTEM_HEADER:     
        DEBUG.println("UDP SYSTEM HEADER");      
        if (atoi(OG_data[1]) != 0){
      ...
      case RESET_HEADER:
        DEBUG.println("UDP RESET HEADER");      
        ESP.restart(); 
        return true;
      break;
...
void checkEEPROM(){
  ...
  {
    DEBUG.print("\r\nNew ESP32 detected, saving defaults to EEPROM\r\n");
  ...
  {
    DEBUG.print("\r\nLoading grade config from EEPROM\r\n");
